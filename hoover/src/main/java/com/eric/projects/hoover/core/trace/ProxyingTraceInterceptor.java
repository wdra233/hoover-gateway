...
public class ProxyingTraceInterceptor {
    ...
    public ProxyingTraceInterceptor(HooverProperties hooverProperties, TraceInterceptor traceInterceptor) {
        this.hooverProperties = hooverProperties;
        this.traceInterceptor = traceInterceptor;
    }

    public String generateTraceId() {
        return hooverProperties.getTracing().isEnabled() ? randomUUID().toString() : null;
    }
    ...
    protected void runIfTracingIsEnabled(Runnable operation) {
        if (hooverProperties.getTracing().isEnabled()) {
            operation.run();
        }
    }
}
